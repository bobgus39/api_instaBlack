...
const insertUserModel = async (username, email, password, defaultImg) => {
    let connection;
    const description = `Bienvenid@  ${username}!!`;
    try {
        ...
        await connection.query(
            ...
            [username, email, hashedPass, defaultImg]
        );

        //insertamos el post por defecto

        await connection.query(
            `INSERT INTO posts( description, photo, userId) VALUES( ?, ?, ?)`,
            [description, defaultImg, 1]
        );
    } finally {
